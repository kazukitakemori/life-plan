...
const WORK_DESIGN_HELP_ITEMS: Record<'detail' | 'simple', string[]> = {
  simple: [
    ...
    '残る世帯主・配偶者の就労収入は、収入の働き方のまま試算します（簡易設計では変更できません）。',
    '遺族年金・老齢年金・児童手当などは詳細設計と同じルールで自動計算します。',
    '遺族年金の「生計維持」（生計同一・収入要件）や初診日を伴う傷病死亡など、現在の入力だけで確定できない要件は別途確認が必要です。',
  ],
  detail: [
    '亡くなった人の給与・事業などは含めません。',
    ...
    '2028年4月以降は、配偶者が高収入で遺族基礎年金を受けられない場合など、子自身が受け取れる範囲の拡大を反映します。死別後の再婚・養子縁組・元配偶者が子を引き取る等の将来の家族状態は未入力のため自動推定しません。',
    '残る世帯主・配偶者が老齢基礎・老齢厚生年金を受け取り始めると、年金の見込み額を額面として足します。',
    '遺族厚生年金は、確認できる受給要件を満たす場合に、亡くなった人の老齢厚生（報酬比例）を基に自動計算します。',
    '遺族の生計維持は、原則として生計同一に加え前年収入850万円未満または所得655.5万円未満等の確認が必要です。現在の入力だけで個別認定まで確定できないため、該当する場合は別途確認してください。',
    '厚生年金加入中に初診日がある傷病で、初診から5年以内に亡くなる要件は、初診日・死因を保存していないため自動判定しません。',
    '60〜64歳の元国民年金被保険者については、日本国内住所の確認が必要ですが、成人の住所を保存していないため、その要件だけでは遺族基礎年金を自動計上しません。',
    '2028年4月以降の死亡分割は、婚姻・内縁関係の期間中の厚生年金記録を分割して将来の老齢厚生年金を増やす仕組みですが、関係開始時期と請求状況を保存していないため自動反映しません。',
    '2028年4月以降の子の遺族基礎年金は現在登録されている家族関係で試算します。死別後の再婚・養子縁組・引取りなど将来の家族関係の変化は予測しません。',
    '在職中の死亡で加入月数が300月未満なら300月とみなします。',
    '残る配偶者が65歳以上で自身の老齢厚生を受けるときは、自身の老齢厚生を全額優先し、遺族厚生は差額のみです。',
    '配偶者の受給期間は、死亡時期・年齢・性別・子の有無に応じて、現行制度と2028年4月改正後の経過措置を切り替えて判定します。',
    '2028年改正の5年有期給付後の継続給付は、障害要件を確認できる場合を除き、政令の所得基準額を確定値として持てない間は自動計上しません。そのため必要保障額は多めに出る場合があります。',
    '中高齢寡婦加算は、対象条件と2028年改正後の段階的な見直しを反映して判定します（非課税）。',
  ],
};
