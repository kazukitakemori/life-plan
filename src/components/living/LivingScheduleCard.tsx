...
export function LivingScheduleCard({
  ...
  const monthlyTotal = calcMonthlyEquivalentMan(schedule.items);
  const emergencyTotal = calcMonthlyEmergencyMan(schedule.items);
  const itemCount = schedule.items.length;
  const firstItem = schedule.items[0];

  const updateItem = (itemId: string, updated: LivingExpenseItem) => {
    onChange({
      ...schedule,
    ...
    <div className="living-schedule-card">
      <div className="living-table">
        <div className="living-table-header">
          ...
          <div className="living-table-header-cell living-col-emergency">
            万が一時の金額（税込）
          </div>
        ...
        >
                    ...
                    <>
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (value === 'lifetime') {
                            onChange({ ...schedule, endMode: 'lifetime' });
                          } else {
                            onChange({
                              ...schedule,
                              endAge: Number(value),
          ...
          {schedule.items.map((item, index) => (
            <div
              key={item.id}
              className={`living-item-row ${dragItemId === item.id ? 'living-item-row--dragging' : ''}`}
              onDragOver={(e) => {
                if (index === 0) return;
                e.preventDefault();
              }}
            ...
            >
              ...
              <div className="living-table-cell living-col-emergency">
                <div className="living-amount-field">
                  <input
                    type="number"
                    className="amount-input"
                    value={item.emergencyAmountMan}
                    min={0}
                    step={0.1}
                    onChange={(e) =>
                      updateItem(item.id, {
          ...
          <div className="living-summary-row">
            ...
            <div className="living-summary-emergency">
              {formatManAmount(emergencyTotal)}
            </div>
          </div>
        </div>
      </div>
