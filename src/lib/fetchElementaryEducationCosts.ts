import {
  buildElementaryFetchedAmounts,
  getElementaryFeeSchedule,
} from '../data/elementaryCostReference';
import { buildElementaryReferenceDetail } from './educationReferenceDetail';
...
export interface ElementaryCostFetchInput {
...
export function fetchElementaryEducationCosts(
  input: ElementaryCostFetchInput,
  ...
  const schedule = getElementaryFeeSchedule(entry.schoolType);
  const { tuitionAnnual, otherExpenses } = buildElementaryFetchedAmounts(schedule);
  ...
  costs.referenceDetail = buildElementaryReferenceDetail({
    areaLabel,
    schedule,
    costs,
