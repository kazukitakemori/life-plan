...
function migrateLegacyNursingFields(
  ...
  return {
    head,
    // 既存データでは従来この初期値が「設定済み」として扱われていたため維持する。
...
function migrateNursingDesign(
  ...
  return createDefaultSecondLifeNursingDesign({
    // configured が存在しない旧データは、既存設定を壊さないよう設定済みとして扱う。
    configured:
...
export function migrateSecondLifeState(
  ...
  return {
    ...defaults,
    ...rest,
    // 旧データにはこのフラグがないため、従来の選択内容を設定済みとして維持する。
    housingConfigured:
      typeof value.housingConfigured === 'boolean'
        ? value.housingConfigured
        : true,
    livingConfigured:
      typeof value.livingConfigured === 'boolean'
        ? value.livingConfigured
        : true,
    startAge:
      typeof value.startAge === 'number' && value.startAge >= 60
        ? value.startAge
