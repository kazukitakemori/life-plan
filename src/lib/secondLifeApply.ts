...
import type { LifeEventSource, LifeEventState } from '../types/lifeEvent';
import {
...
export { SECOND_LIFE_HOUSING_EVENT_LABEL } from './lifeEventSource';
...
function upsertOneTimeLifeEvent(
  ...
  const without = entries.filter(
    (entry) =>
      getSecondLifeManagedLifeEventSource(entry) !== source &&
      entry.label !== label,
  );
  if (amountMan <= 0) {
    return {
