...
import { calcCurrentHousingLoanYearEndBalanceYen } from './currentHousingLoan';
...
interface HousingLoanDeductionBalanceOverride {
...
function calcHousingLoanDeductionFromSettingsYen(
  ...
  balanceOverride?: HousingLoanDeductionBalanceOverride,
): number {
  ...
  const loanAmountMan =
    balanceOverride?.loanAmountMan ??
    calcHousingLoanTotalAmountMan(property, loan, loanAmountOptions);
  ...
  const yearEndBalanceYen =
    balanceOverride?.yearEndBalanceYen ??
    calcHousingLoanDeductionEligibleYearEndBalanceYen(
      property,
      loan,
      memberAge,
...
export function calcLoanEntryHousingLoanDeductionYen(
  ...
  // 月々返済額だけの入力では年末残高を復元できないため、控除額は推測しない。
  if (isLoanMonthlyRepaymentMode(entry)) return 0;
  ...
  let balanceOverride: HousingLoanDeductionBalanceOverride | undefined;
  if (isLoanCurrentBalanceMode(entry)) {
    const effectiveReferenceDate =
      referenceDate ?? new Date(referenceYear, 0, 1);
    let yearEndBalanceYen = calcCurrentHousingLoanYearEndBalanceYen(
      entry,
      effectiveReferenceDate,
      calendarYear,
    );
    if (jointDebtPrimaryShare != null) {
      const sharePct = deductionShareOverride ?? jointDebtPrimaryShare;
      yearEndBalanceYen = Math.floor(
        (yearEndBalanceYen * Math.max(0, Math.min(100, sharePct))) / 100,
      );
    }
    balanceOverride = {
      loanAmountMan: entry.currentBalanceMan,
      yearEndBalanceYen,
    };
  }
